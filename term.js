...
function refresh_UI(){
	...
	for(var gauge=0;gauge<gauges.length;gauge++){
		if(gauge_buf[gauge]!=gauge_buf_old[gauge]){
			gauges[gauge].refresh(gauge_buf[gauge]);
			gauge_buf_old[gauge]=gauge_buf[gauge];
		}
	}
}
...
function compute(dat){
	...
	switch(dat[DATA_TYPE]){
		...
		case TT_GAUGE_CONF:
			...
			gauges[gauge_num].refreshTitle(str);
			gauges[gauge_num].refresh(gauge_min,gauge_max);
		break;
		case TT_CHART_CONF:
